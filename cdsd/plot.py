...
# matplotlib 和 seaborn 延迟导入，只在实际绘图时才导入
# 这样可以避免在 Jupyter 初始化时就加载绘图库导致 crash
plt = None
sns = None
HAS_BASEMAP = False
...
def _ensure_metrics():
    """惰性导入 metrics"""
    global mcc_latent
    if mcc_latent is None:
        from cdsd.metrics import mcc_latent as _mcc_latent
        mcc_latent = _mcc_latent

def _ensure_dataloader():
    """惰性导入 DataLoader"""
    global DataLoader
    if DataLoader is None:
        from cdsd.data_loader import DataLoader as _DataLoader
...
class Plotter:
        ...
        if not learner.no_gt:
            if learner.latent:
                ...
                if learner.debug_gt_z:
                    ...
                    _ensure_metrics()  # 確保 mcc_latent 已導入
                ...
                if getattr(learner.hp, 'nonlinear_mixing', False):
    ...
    def plot_compare_prediction(self, x, x_past, x_hat, coordinates: np.ndarray, path: str):
        ...
        """
        # 此函數需要 Basemap 來繪製地圖，SAVAR 數據不需要
        if not HAS_BASEMAP:
            print("Skipping plot_compare_prediction: Basemap not available")
            return
    ...
    def plot_regions_map(self, w_adj, coordinates: np.ndarray, iteration: int,
        ...
        Args:
            ...
            path: path where to save the plot
        """
        # 此函數需要 Basemap 來繪製地理區域，SAVAR 數據不需要
        if not HAS_BASEMAP:
            print("Skipping plot_regions_map: Basemap not available")
            return
