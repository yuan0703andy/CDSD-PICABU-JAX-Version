"""
JAX/Optax Functional 版本的 TSDCD 潛變量模型
...
=== PICABU 修改版 ===
修改內容：
1. tsdcd_forward 返回更多資訊供 PICABU 損失使用
2. 新增 px_std, logvar_decoder, z_mu, z_std, pz_mu, pz_std
"""
...
def tsdcd_forward(
    ...
    返回額外資訊：
    - px_std: 解碼器標準差
    - logvar_decoder: 解碼器對數方差（用於 CRPS）
    - z_mu, z_std: 編碼器輸出的均值和標準差
    - pz_mu, pz_std: 轉移模型的先驗均值和標準差
    """
    batch = x.shape[0]
    key = rng
    ...
    return {
        ...
        "mask": mask,
        # PICABU 新增輸出
        "px_std": px_std,                                    # 解碼器標準差 (B,d,d_x)
        "logvar_decoder": params["autoencoder"]["logvar_decoder"],  # (d_x,)
        "z_mu": q_mu_y,                                      # 編碼器均值 (B,d,d_z)
        "z_std": q_std_y,                                    # 編碼器標準差 (B,d,d_z)
        "pz_mu": pz_mu,                                      # 轉移先驗均值 (B,d,d_z)
        "pz_std": pz_std,                                    # 轉移先驗標準差 (B,d,d_z)
    }
...
def _test():
    """測試模型和 JIT 編譯"""
    print("Testing LatentTSDCD functional JAX implementation (PICABU version)...")
    print("="*60)

    # 注意：mask 形狀是 (batch, tau+1, d_z, d_z)
    # transition 期望 (batch, tau+1, d*d_z, d*d_z)
    # 當 d=1 時兩者相等，所以用 d=1 測試
    cfg = TSDCDConfig(
        d=1,           # 使用 d=1 以匹配 mask 形狀
        d_x=4,
        d_z=3,         # 增加 d_z 以保持模型複雜度
        tau=1,
        num_layers=2,
        num_hidden=64,
        num_layers_mixing=2,
    ...
    # 3) 測試 forward 輸出（檢查 PICABU 新增欄位）
    print("\n2) Testing forward output (PICABU extensions)...")
    key, sub = jax.random.split(key)
    out = tsdcd_forward(state.params, cfg, sub, x, y, deterministic=False)
    
    print(f"   Original outputs:")
    print(f"     elbo: {float(out['elbo']):.4f}")
    print(f"     recons: {float(out['recons']):.4f}")
    print(f"     kl: {float(out['kl']):.4f}")
    print(f"     px_mu shape: {out['px_mu'].shape}")
    
    print(f"   PICABU new outputs:")
    print(f"     px_std shape: {out['px_std'].shape}")
    print(f"     logvar_decoder shape: {out['logvar_decoder'].shape}")
    print(f"     z_mu shape: {out['z_mu'].shape}")
    print(f"     z_std shape: {out['z_std'].shape}")
    print(f"     pz_mu shape: {out['pz_mu'].shape}")
    print(f"     pz_std shape: {out['pz_std'].shape}")

    # 4) 測試 train_step (JIT)
    print("\n3) Testing train_step (with JIT)...")
    ...
    print("  PICABU extensions verified: px_std, logvar_decoder, z_mu, z_std, pz_mu, pz_std")
    print("="*60)
